...
class IGCParser {
    async parseIGC(date, track) {
        ...
        try {
            ...
            this.processIGCContent(date, igcText, track.path);
        } catch (error) {
            console.error(`Error downloading or processing IGC file: ${error}`);
        }
    }

    processIGCContent(date, igcText, path) {
        ...
        lines.forEach(line => {
            ...
            if (match) {
                ...
                const time = dayjs(`${date} ${timeStr.slice(0, 2)}:${timeStr.slice(2, 4)}:${timeStr.slice(4, 6)}`);
                path.addPoint(longitude, latitude, altitude, time);
            }
        });
    }
